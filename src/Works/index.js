...
const WorkWall = styled.img`transition: transform 0.5s;`;
...
const WorkItem = styled(Link)`
	...
	position: relative;
	width: 100%;
	padding-top: ${props => (props.shrink ? `${100 / 3 * 1}%` : `${100 / 1.618 * 1}%`)};
	background: #f6f6f9;
	&:hover {
		${WorkWall}, ${WorkWall_v} {
			transition: transform 0.5s;
...
const WorkLink = ({ location, shrink = false }) => {
	return [
		<WorkItem to="/jiggle" comp={location.pathname == '/jiggle'} shrink={shrink}>
		...
		<WorkItem to="/twitch" comp={location.pathname == '/twitch'} shrink={shrink}>
			<div className="ratio__inner">
		...
		<WorkItem to="/CONVIoT" comp={location.pathname == '/CONVIoT'} shrink={shrink}>
			<div className="ratio__inner">
